...
const QOCheckout = () => {
  ...
  const content = {
    ...
    ko: {
      ...
      qrPayment: "QR 결제",
      orderNotes: "주문 메모",
      notesPlaceholder: "주문에 대한 특별 요청사항...",
      terms: "이용약관",
      privacy: "개인정보처리방침",
  ...
  const orderData = {
    items: [
      { name: { en: 'Avocado Toast Supreme', ko: '슈프림 아보카도 토스트' }, quantity: 2, price: { AUD: 45.00, KRW: 65000 } },
      { name: { en: 'Oat Milk Latte', ko: '오트밀크 라떼' }, quantity: 1, price: { AUD: 7.20, KRW: 10500 } },
      { name: { en: 'Pancake Stack', ko: '팬케이크 스택' }, quantity: 1, price: { AUD: 18.80, KRW: 27000 } }
    ],
    subtotal: { AUD: 71.00, KRW: 102500 },
    tax: { AUD: 7.10, KRW: 10250 },
    serviceCharge: { AUD: 3.55, KRW: 5125 },
    discount: { AUD: 7.10, KRW: 10250 },
  ...
  const handlePlaceOrder = async () => {
    ...
    setTimeout(() => {
      alert('Order placed successfully! (Navigate to QO-C006)');
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="p-4 pb-32 space-y-6">
        ...
        <div className="bg-white rounded-xl p-4">
          ...
          <div className="border-t pt-2 space-y-1">
            <div className="flex justify-between text-sm text-slate-600">
              <span>Subtotal</span>
              <span>{currencySymbol}{orderData.subtotal[currencyCode].toLocaleString()}{selectedLanguage === 'ko' ? '원' : ''}</span>
            </div>
            <div className="flex justify-between text-sm text-green-600">
              <span>Discount</span>
              <span>-{currencySymbol}{orderData.discount[currencyCode].toLocaleString()}{selectedLanguage === 'ko' ? '원' : ''}</span>
            </div>
          ...
          <div className="mt-3 flex items-center justify-between text-sm text-slate-500 bg-slate-50 rounded-lg p-2">
            <div className="flex items-center space-x-1">
              <Clock className="w-4 h-4" />
              <span>{currentContent.estimatedTime}: {orderData.estimatedTime} {currentContent.minutes}</span>
            </div>
            <span>{orderData.items.reduce((sum, item) => sum + item.quantity, 0)} {currentContent.items}</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <div className="text-sm font-medium">{currentContent.dineIn}</div>
            </button>
            <button
            ...
            >
              ...
              <div className="text-sm font-medium">{currentContent.takeaway}</div>
            </button>
          </div>
        ...
        <div className="bg-white rounded-xl p-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-slate-900">{currentContent.customerInfo}</h2>
            <div className="flex space-x-2">
              <button
                onClick={() => setCustomerInfo(prev => ({ ...prev, isGuest: true }))}
              ...
              >
                {currentContent.guestCheckout}
              </button>
              <button
                onClick={() => setCustomerInfo(prev => ({ ...prev, isGuest: false }))}
                className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${
                  !customerInfo.isGuest
                    ? 'bg-slate-900 text-white'
                    : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-3 py-2 bg-slate-50 rounded-lg border-0 focus:ring-2 focus:ring-slate-900 ${
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={selectedLanguage === 'ko' ? '010-1234-5678' : '+61 400 000 000'}
                />
              </div>
              {errors.phone && (
                <div className="mt-1 flex items-center space-x-1 text-red-600 text-xs">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-200">
        <button
          ...
          className="w-full bg-slate-900 text-white py-4 rounded-xl font-semibold text-lg flex items-center justify-center space-x-2 hover:bg-slate-800 transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {isProcessing ? (
            <>
